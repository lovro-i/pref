...
public class FactorTable extends SparseFactorTableBase {
  ...
  /**
   * If the low-order four-bits are zero, then the table represents a directed
   * deterministic function, with non-zero entries indicated by the values in
   * {@link #_sparseIndexToJointIndex} but weights and values are not stored
   * explicitly because all the non-zero weights will be one.
   * <p>
  ...
  /**
   * If set, the table stores energies in dense representation in
   * {@link #_denseEnergies}.
   */
  static final int DENSE_ENERGY = 0x1;

  /**
   * If set, the table stores weights in dense representation in
   * {@link #_denseEnergies}.
   */
  static final int DENSE_WEIGHT = 0x2;

  /**
   * If set, the table stores energies in sparse representation in
   * {@link #_sparseEnergies} and mapping from sparse to joint indices in
   * {@link #_sparseIndexToJointIndex}.
   */
  static final int SPARSE_ENERGY = 0x4;

  /**
   * If set, the table stores weights in sparse representation in
   * {@link #_sparseWeights} and mapping from sparse to joint indices in
   * {@link #_sparseIndexToJointIndex}.
   */
  static final int SPARSE_WEIGHT = 0x8;

  /**
   * If set, the table stores mapping from sparse indices to joint table indices
   * in {@link #_sparseIndices}.
   */
  ...
  /**
   * Set if {@link #isDeterministicDirected()} has been invoked since the last
   * time the values or representation of the table were changed.
   */
  ...
  /**
   * Constructs a new table by converting the contents of {@code other} table
   * using {@code converter} whose "from" domains must match {@code other}'s
   * domains. New table will have same representation as {@code other}.
   */
  ...
  /**
   * Constructs a new table with given representation by converting the contents
   * of {@code other} table using {@code converter} whose "from" domains must
   * match {@code other}'s domains.
   */
  ...
  /**
   * Create a directed deterministic factor table that marginalizes out the
   * specified subdomain of the joint input domain.
   * <p>
   * @param outputDomainIndex a number in the range [0,
   * inputDomain.getDimensions() - 1] that specifies which subdomain to produce
   * as the first dimension and output.
  ...
  /**
   * Constructs a new factor table that is the product of all of the given
   * tables.
   * <p>
   * Invokes {@link #product(ArrayList, FactorTableRepresentation)} with null
   * representation argument.
   */
  public static @Nullable
  IFactorTable product(ArrayList<Tuple2<IFactorTable, int[]>> entries) {
   ...
   * tables.
   *
   * @param entries maps factor tables to an array of index mappings that
   * indicates where each dimension in the table is located in the table to be
   * constructed. Thus the index mapping array for a given factor table must
   * have length equal to the factor table's dimensions and each entry must be
   * in the range [0,N] where N is one less than the size of the new table.
   * Every value from 0 to N must be represented in at least one index mapping
   * array. N can be any number between the sum of the number of dimensions of
   * all factors when the factors do not share any dimensions in common, or the
   * number of dimensions of the largest factor. When two tables both map a
   * dimension to the same dimension in the target table, the domains must
   * match.
   * <p>
   * @param representation is the representation to use for the table to be
   * constructed. If null, the representation will be set to either
   * {@link FactorTableRepresentation#DENSE_ENERGY} or
   * {@link FactorTableRepresentation#SPARSE_ENERGY} based on the density of the
   * tables.
   * <p>
   * @return Newly constructed table.
   */
  public static @Nullable
  IFactorTable product(
   ...
   */
  /**
   * Override the default serialization to decrease size of serialized
   * representation.
   */
  ...
  @Deprecated
  public static @Nullable
  IFactorTable deserializeFromXML(String docName) {
