...
public class SparseFactorTable extends SparseFactorTableBase implements IFactorTable {
  ...
  /**
   * Like {@link #sparseIndexToIndices(int)} but returns actual internal indices
   * array (which must not be modified!).
   */
  ...
  /**
   * If {@link #hasSparseIndices()} this will recompute their values based on
   * the current index entries.
   */
