...
public abstract class SparseFactorTableBase extends FactorTableBase implements IFactorTable {
  ...
  /**
   * Set if table is directed and has been conditionally normalized (so that the
   * total weight for any two inputs is the same).
   */
   ...
   * by {@link #getRepresentation()} and
   * {@link #setRepresentation(FactorTableRepresentation)}.
   * <p>
   ...
   * <p>
   * When table has no sparse representation, this is returned as the
   * {@link #sparseSize()}.
   */
  int _nonZeroWeights;
  /**
   * Same information as FactorTable's _sparseIndexToJointIndex but instead of
   * storing joint indices stores arrays of element indices.
   */
  int[][] _sparseIndices = ArrayUtil.EMPTY_INT_ARRAY_ARRAY;

  /**
   * Information computed about the table based on its values. This field is
   * zeroed out whenever table weights or energies are changed.
   * <p>
  ...
  /**
   * Throws exception with message indicating an attempt to normalize a directed
   * table whose weights for some input adds up to zero.
   *
  ...
  /**
   * Returns an exception with message indicating an attempt to normalize an
   * undirected table whose weights add up to zero.
   */
