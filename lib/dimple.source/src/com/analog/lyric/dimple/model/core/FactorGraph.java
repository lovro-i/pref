...
public class FactorGraph extends FactorBase {
  ...
  /**
   * Flags that are reserved for use by this class and should not be used by
   * subclasses when invoking {@link #setFlags(int)} or {@link #clearFlags()}.
   */
   ...
   */
  /**
   * Environment for this graph determined at construction. Unless the
   * constructor specifies a different one, this will be the value of
   * {@link DimpleEnvironment.active} when the object was constructed.
   */
  private final DimpleEnvironment _env;

  private volatile @Nullable
  IDimpleEventSource _eventAndOptionParent;
  ...
  private @Nullable
  ISchedule _schedule = null;
  private @Nullable
  IScheduler _explicitlyAssociatedScheduler = null;
  private @Nullable
  IScheduler _solverSpecificDefaultScheduler = null;
  ...
  private @Nullable
  Class<? extends ISolverFactorGraph> _schedulerSolverClass;
  private @Nullable
  IFactorGraphFactory<?> _solverFactory;
  private @Nullable
  ISolverFactorGraph _solverFactorGraph;
  private @Nullable
  LoadingCache<Functions, JointFactorFunction> _jointFactorCache = null;
   ...
   * {@inheritDoc}
   * <p>
   * By default this returns {@link #getParentGraph()} if non null, and
   * otherwise returns the value of {@link #getEnvironment()} (which means that
   * the environment is the default root of the event source hierarchy), but it
   * may be also be set to another value using
  ...
  @Override
  public @Nullable
  IDimpleEventSource getOptionParent() {
  ...
  @Override
  public @Nullable
  IDimpleEventSource getEventParent() {
    return _eventAndOptionParent;
  }
   ...
   * Sets event listener.
   * <p>
   * Sets the value to be returned by {@link #getEventListener()}. This should
   * only be set on a root graph.
   * <p>
   * @param listener is the event listener to be used for this graph and all of
   * its contents. May be set to null to turn off listening.
   * @deprecated As of release 0.07, this functionality has been moved to
   * {@link DimpleEnvironment#setEventListener}.
   * @since 0.06
   */
  @Deprecated
   ...
   * {@link #getOptionParent()}/{@link #getEventParent()} to {@code parent},
   ...
   * {@inheritDoc}
   * <p>
   * In addition to setting the value of @link #getParentGraph()}, this will
   * also reset the value of {@link #getOptionParent()} and
   * {@link #getEventParent()} to the new parent graph if non-null and otherwise
   * the value of {@link #getEnvironment()}.
   */
  @Override
   ...
   */
  private @Nullable
  ISolverFactorGraph setSolverFactorySubGraph(@Nullable ISolverFactorGraph parentSolverGraph,
   ...
   * <p>
   * Same as {@link #setSolverFactory(IFactorGraphFactory)} but does not accept
   * a null factory and returns a non-null solver graph.
   * <p>
   * @param factory
   * @since 0.06
   */
   ...
   *
   * @param factory is used to construct the solver graph and will be used to
   * create solver state for any nodes subsequently added to the graph. If null,
   * then the solver will be cleared from the graph.
   * @return The newly created solver graph or else null if {@code factory} was
   * null.
   */
  public @Nullable
  <SG extends ISolverFactorGraph> SG setSolverFactory(@Nullable IFactorGraphFactory<SG> factory) {
  ...
  /**
   * True if variable is one of this graph's boundary variables or is owned by
   * this graph or one of its subgraphs.
   */
  ...
  /**
   * Slower version of {@link #OwnsDirectly} just used for checking correctness
   * in assertion.
   */
   ...
   * @throws DimpleException if the variable is still connected to some factor
   * or if the variable is not owned by this graph.
   * @see #remove(Factor)
   */
  ...
  /**
   * Merges {@code factors} into a single joint factor over the given set of
   * variables.
   * <p>
   * @param variables specifies the variables and the order in which they will
   * appear in the new joint factor. This may include variables that are not in
   * any of the specified {@code factors} but must not omit any variable that
   * appears in one of the {@code factors} nor should it repeat any variable.
   * @param factors specifies the factors to be merged. If empty, this will add
   * a new uniform factor over the specified variables.
   * @return the new joint factor. If {@code factors} has a single entry with
   * the specified {@code variables} in the specified order, this will simply
   * return that factor without modifying the graph.
   */
  ...
  public @Nullable
  IScheduler getExplicitlySetScheduler() {
    return _explicitlyAssociatedScheduler;
  }
  ...
  /**
   * Add a new subgraph generated from specified template graph attached to
   * given boundary variables.
   * <p>
  ...
  /**
   * Initializes components of model, and if a solver is set, also initializes
   * the solver.
   * <p>
   * Does the following:
   * <ol>
   * <li>Invokes {@link #notifyListenerChanged()} to reconfigure event
   * notifications.
   * <li>Initializes non-boundary model variables contained directly in the
   * graph (not in subgraphs) by calling {@link Variable#initialize()} on each.
   * <li>If not {@link #hasParentGraph()}, initializes boundary variables in the
   * same fashion.
   * <li>Initializes all model factors contained directly in the graph by
   * calling {@link Factor#initialize()} on each.
   * <li>Initializes nested graphs by invoking this method recursively on each.
   * <li>Finally, if {@link #getSolver()} is not null, invokes
   * {@link ISolverFactorGraph#initialize()} on the solver graph to initialize
   * solver state. The solver is responsible for initializing its component
   * variables, factors and any other state.
   * </ol>
   */
  @Override
   ...
   * given any two nodes in the graph there is at most one unique path between
   * them.
   * <p>
   * @see #isTree()
   * @see #isForest(int)
  ...
  /**
   * True if the graph is consists of a set of one or more disjoint trees when
   * considering nodes down to the specified {@code relativeNestingDepth} of
   * subgraphs. That is, given any two nodes in the graph that are no deeper
   * than the specified depth below the root graph, there no more than one
  ...
  /**
   * True if the graph is singly connected and not disjoint. That is, given any
   * two nodes in the graph there is exactly one unique path between them.
   * <p>
   * @see #isForest()
   * @see #isTree(int)
   */
  ...
  /**
   * True if the top-level of the tree -- ignoring the contents of any subgraphs
   * -- is a tree.
   * <p>
  ...
  /**
   * True if the graph is singly connected and not disjoint when considering
   * nodes down to the specified {@code relativeNestingDepth} of subgraphs. That
   * is, given any two nodes in the graph that are no deeper than the specified
   * depth below the root graph, there is exactly one unique path between them.
   * <p>
   * @see #isForest(int)
   * @see #isTree()
   */
   ...
   *
   * @param i an index in the range [0, {@link #getBoundaryVariableCount()} -
   * 1].
   * @since 0.05
   */
  ...
  /**
   * Returns the number of non-boundary variables contained directly in this
   * graph (i.e. not in subgraphs).
   *
  ...
  /**
   * Returns count of variables that would be returned by
   * {@link #getVariables()}.
   */
  ...
  /**
   * Returns list of all variables in the graph including those in nested
   * graphs.
   */
   ...
   * down to specified {@code relativeNestingDepth} below this graph, where a
   * nesting depth of zero indicates that nested graphs should not be included.
   */
  ...
  /**
   * Returns count of factors that would be returned by
   * {@link #getFactors(int)}.
   */
  ...
  /**
   * Returns a newly constructed collection containing all factors within the
   * specified nesting depth and subgraphs at the specified depth.
   * <p>
   ...
   * <p>
   * @param relativeNestingDepth is a non-negative number indicating how many
   * levels of subgraphs will be explored. Factors at the specified relative
   * depth below the starting graph or less will be included. Subgraphs at the
   * exact relative depth will be included, but <em>not</em> those at shallower
   * depth.
   * <p>
  ...
  /**
   * Returns newly constructed collection containing all of the factors and
   * subgraphs that are directly owned by this graph.
   */
  ...
  private @Nullable
  Object getObjectByNameOrUUIDWithoutRecurse(String string) {
  ...
  public @Nullable
  Object getObjectByName(@Nullable String name) {
    Object o = null;
  ...
  public @Nullable
  FactorGraph getGraphByName(String name) {
    FactorGraph fg = null;
    Object o = getObjectByName(name);
  ...
  public @Nullable
  Variable getVariableByUUID(UUID uuid) {
    Variable v = null;
  ...
  public @Nullable
  Factor getFactorByUUID(UUID uuid) {
    Factor f = null;
    Object o = getObjectByUUID(uuid);
    if (o != null
      && o instanceof Factor) {
  ...
  public @Nullable
  FactorGraph getGraphByUUID(UUID uuid) {
    FactorGraph fg = null;
    Object o = getObjectByUUID(uuid);
    if (o != null
  ...
  public String toString() {
    ...
    String s = String.format("FactorGraph [%s] Variables:(o:%d  b:%d  ch:%d)  Functions:(o:%d ch:%d) Graphs:%d", getQualifiedLabel(), ownedVariables, boundaryVariables, childVariables, ownedFunctions, childFunctions, subGraphs);
    return s;
  ...
  public String getAdjacencyString() {
    ...
    for (Factor fn : allFunctions) {
      ...
      for (int i = 0, end = fn.getSiblingCount(); i < end; i++) {
        ...
        if (vParent != null
          && // can happen with boundary variables
  ...
  public @Nullable
  IFactorGraphFactory<?> getFactorGraphFactory() {
  ...
  public TreeSet<Edge> getEdges() {
    ...
    for (Factor factor : factors) {
      ...
      for (int i = 0, end = factor.getSiblingCount(); i < end; i++) //for(Port port : ports)
      {
  ...
  protected @Nullable
  HashMap<Integer, ArrayList<Variable>> _variableGroups;
  ...
  public @Nullable
  ArrayList<Variable> getVariableGroup(int variableGroupID) {
