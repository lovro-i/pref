...
public abstract class Variable extends Node implements Cloneable, IDataEventSource {
  ...
  Object _input = null;
  protected @Nullable
  Object _fixedValue = null;
  protected String _modelerClassName;
  protected @Nullable
  ISolverVariable _solverVariable = null;
  ...
  Variable clone() {
    /**
     * *****
     * NOTE: Any derived class that defines instance variables that are objects
     * (rather than primitive types) must implement clone(), which must first
     * call super.clone(), and then deep-copy those instance variables to the
     * clone.
		 ******
     */
  ...
  /**
   * Returns the solver-specific variable instance associated with this model
   * variable if any.
   */
  @Override
  public @Nullable
  ISolverVariable getSolver() {
   ...
   * <p>
   * Clears {@link #isDeterministicInput()} and
   * {@link #isDeterministicOutput()}. Does NOT invoke solver variable
   * initialize.
   */
  @Override
  ...
  /**
   * Returns the solver-specific variable instance associated with this model
   * variable if it is an instance of the specified {@code solverVariableClass},
   * otherwise returns null.
   */
  public @Nullable
  <T extends ISolverVariable> T getSolverIfType(Class<? extends T> solverVariableClass) {
    final ISolverVariable svar = _solverVariable;
    T result = null;
   ...
   * Returns the solver-specific variable instance associated with this model
   * variable if it is an instance of the specified {@code solverVariableClass}
   * and has {@link SNode#getParentGraph()} equal to {@code solverGraph},
   * otherwise returns null.
   */
  public @Nullable
  <T extends ISolverVariable> T getSolverIfTypeAndGraph(
   ...
   */
  /**
   * Creates a new variable that combines the domains of this variable with
   * additional {@code variables}.
   * <p>
   * For use by {@link FactorGraph#join(Variable...)}. Currently only supported
   * for {@link Discrete} variables.
   * <p>
   * @param variables specifies at least one additional variables to join with
   * this one. As a convenience, this may begin with this variable, in which
   * case there must be at least one other variable.
   *
   * @category internal
   */
  @Internal
