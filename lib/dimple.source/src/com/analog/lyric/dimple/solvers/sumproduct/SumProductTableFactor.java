...
public class SumProductTableFactor extends STableFactorDoubleArray
  ...
  @Internal
  @Nullable
  FactorTableUpdateSettings getFactorTableUpdateSettings() {
  ...
  /**
   * Enables use of the optimized update algorithm on this factor, if its degree
   * is greater than 1. The optimized update algorithm is employed only when all
   * of the factor's edges are updated together with an update call. If the
   * schedule instead uses update_edge, the algorithm is not used.
   * <p>
  ...
  /**
   * Reverts to the default setting for enabling of the optimized update
   * algorithm, eliminating the effect of previous calls to
   * {@link #enableOptimizedUpdate()} or {@link #disableOptimizedUpdate()}.
   * <p>
  ...
  /**
   * Returns the effective update approach for the factor. If the update
   * approach is set to automatic, this value is not valid until the graph is
   * initialized. Note that a factor with only one edge always employs the
  ...
  @Internal
  public @Nullable
  UpdateApproach getAutomaticUpdateApproach() {
