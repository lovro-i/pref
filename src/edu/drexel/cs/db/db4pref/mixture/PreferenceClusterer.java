...
import edu.drexel.cs.db.db4pref.distance.PreferenceSimilarity;
...
/** Class that uses Affinity Propagation to cluster PreferenceSets. */
public class PreferenceClusterer {
  ...
  public PreferenceClusterer(int maxClusters) {
  ...
  private PreferenceClusters cluster(Sample<? extends PreferenceSet> sample, double alpha) {
    ...
    if (samples.size() > maxClusters && maxSim > 0) { // && samples.size() < sample.size()) {      
      ...
      PreferenceClusters sub = cluster(more, alphaDecay * alpha);
    ...
    return new PreferenceClusters(exemplars, samples);
  }
}
