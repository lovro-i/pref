...
public class FullExpand {
  ...
  public FullExpand(Sequence seq) {
  ...
  public FullExpand(FullExpander expander) {
  ...
  private FullExpand(FullExpander expander, Item[] items) {
    this.expander = expander;
  ...
  private FullExpand(FullExpander expander, FullExpand e) {
    this.expander = expander;
  ...
  public FullExpands insertMissing(Item item) {
    ...
    int pos = 0;
    for (int i = 0; i < miss.length; i++) {
      FullExpand ex = new FullExpand(expander, this);
      ex.miss[i]++;      
      
      double p = 0;
      for (int j = 0; j <= miss[i]; j++) {
        p += probability(expander.referenceIndex.get(item), pos);
        pos++;
      }
      expands.add(ex, p);
    }
  ...
  public FullExpands insert(Item e, Item prev) {
    ...
    for (int i = 0; i < n; i++) {
      FullExpand ex = new FullExpand(expander, items1);
      ...
      double p = probability(expander.referenceIndex.get(e), posPrev + 1 + i);
      expands.put(ex, p);
    }
  ...
  public boolean equals(Object o) {
    if (!(o instanceof FullExpand)) return false;
    FullExpand e = (FullExpand) o;
