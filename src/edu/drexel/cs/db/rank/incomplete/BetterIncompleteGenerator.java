...
public class BetterIncompleteGenerator {
  ...
  public BetterIncompleteGenerator(boolean triangle, boolean triangleByRow, int bootstraps) throws Exception {
    ...
    this.attributes = BetterIncompleteAttributes.getAttributes(triangle, triangleByRow, boots);
  ...
  public Instance generateInstance(ItemSet items, int sampleSize, int resampleSize, double phi, Missing missing) {
    Instance instance = new DenseInstance(attributes.size());
    instance.setValue(attributes.indexOf(BetterIncompleteAttributes.ATTRIBUTE_REAL_PHI), phi);
  ...
  public Instances generate(Sample sample, int reps, int threads) throws Exception {
    ...
    this.missing = new Missing(sample);
  ...
  private class Trainer extends Thread {
    ...
    public void run() {
      ...
      while (true) {
        ...
        Instance instance = generateInstance(items, sampleSize, resampleSize, phi, missing);
        add(instance);
      }
  ...
  public static void main(String[] args) throws Exception {
    ...
    BetterIncompleteGenerator generator = new BetterIncompleteGenerator(false, false, 10);
    Instances data = generator.generate(sample, 2, 4);
