...
public class TopIncompleteAttributes {
  ...
  public static final Attribute ATTRIBUTE_TRIANGLE_TOP = new Attribute("triangle_top");
  ...
  static ArrayList<Attribute> getAttributes(boolean triangle, boolean triangleByRow, boolean triangleTop, int boots) {
    ArrayList<Attribute> attributes = new ArrayList<Attribute>();
    if (triangle) attributes.add(ATTRIBUTE_TRIANGLE_NO_ROW);
    if (triangleByRow) attributes.add(ATTRIBUTE_TRIANGLE_BY_ROW);
    if (triangleTop) attributes.add(ATTRIBUTE_TRIANGLE_TOP);
    if (boots > 0) {
